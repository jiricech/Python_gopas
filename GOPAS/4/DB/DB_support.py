...
def btnUpdate_Click(p1):
    ...
    # connection zustalo otevrene - pouziji; v praxi zavrit a znovu otevrit
    id = records[currentRecord]["EmployeeID"]  # PK
    newLastName = txtPrijmeniVar.get()  # nacteni uzivatelem zmenene hodnoty

    cursor.execute("Update Employees Set LastName='" + newLastName + "'" +
                   " Where EmployeeID=" + str(id))  # poslani SQL dotazu pro zmenu dat (prijmeni)
    conn.commit()  # potvrzeni - NUTNE!!

    # znovunacteni dat
    cursor.execute("Select EmployeeID,LastName,FirstName,TitleOfCourtesy,City From Employees")
    records = cursor.fetchall()             
    recordCount = len(records)
    # v praxi nastavit i currentRecord (napr. po delete)
    fillForm(currentRecord)
